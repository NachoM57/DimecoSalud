...
export class NoticiasComponent implements OnInit {
  ...
  transformToHtmlBreaks(text: string): string {
    ...
    formattedText = formattedText.replace(/(¡No ignores las señales de dolor!)/, '<strong>$1</strong>');
    formattedText = formattedText.replace(/(¿Sabías que el sangrado genital durante la menopausia es motivo de consulta médica?)/, '<strong>$1</strong>');
